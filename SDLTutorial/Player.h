...
public:
	...
	bool PlayerMove(SDL_Keycode key, SDL_Renderer * renderer)
	{
		...
		case SDLK_UP:
			testPos.yPos--;
			curObjTexture = playerUp;//IMG_LoadTexture(renderer, IMG_PLAYER_UP);
			break;
		case SDLK_DOWN:
			testPos.yPos++;
			curObjTexture = playerDown;//IMG_LoadTexture(renderer, IMG_PLAYER_DOWN);
			break;
		case SDLK_LEFT:
			testPos.xPos--;
			curObjTexture = playerLeft;//IMG_LoadTexture(renderer, IMG_PLAYER_LEFT);
			break;
		case SDLK_RIGHT:
			testPos.xPos++;
			curObjTexture = playerRight;//IMG_LoadTexture(renderer, IMG_PLAYER_RIGHT);
			break;
		default:
